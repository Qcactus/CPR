...
if DATASET == "movielens_10m":
    ...
    for filename in os.listdir(
            "/Users/qi/Documents/data/netflix/training_set"):
        with open(
                os.path.join("/Users/qi/Documents/data/netflix/training_set",
                             filename)) as f:
            i = f.readline().strip(":\n")
            for l in f:
                splits = l.split(',')
...
def filter_degree(df, min_u_d, min_i_d):
...
for idx in range(len(unbias)):
    unbias.values[idx] = id2idx_u[unbias.values[idx][0]], id2idx_i[
        unbias.values[idx][1]]
for idx in range(len(train)):
    train.values[idx] = id2idx_u[train.values[idx][0]], id2idx_i[
        train.values[idx][1]]
...
df = pd.DataFrame({
    "id": list(id2idx_u.keys()),
    "idx": list(id2idx_u.values())
})
df.to_csv(os.path.join(DATA_DIR, "id2idx_u.csv"), index=False)

df = pd.DataFrame({
    "id": list(id2idx_i.keys()),
    "idx": list(id2idx_i.values())
})
...
def subsample(frac, pow):
    ...
    train_skew = train.sample(frac=frac, weights=str(pow),
                              random_state=0)[["user", "item"]]
    users = train_skew["user"].unique()
    items = train_skew["item"].unique()
    train_missing = train[~(train["user"].isin(users)
                            & train["item"].isin(items))]
    train_skew = pd.concat([train_skew, train_missing])
    degree_plot({"train_" + str(pow): train_skew}, FIG_DIR)
    train_skew.to_csv(os.path.join(DATA_DIR, "train_" + str(pow) + ".csv"),
                      index=False)
