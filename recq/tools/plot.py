...
def plot_save(
    ...
    mkdir(dirpath)
    plt.savefig(os.path.join(dirpath, filename + ".png"), dpi=300, bbox_inches="tight")
    plt.clf()
...
def degree_plot(datasets, dirname):
    ...
    for name, dataset in datasets.items():
        ...
        if plot["Item Degree"].max() < 100:
            bins = "auto"
        sns.histplot(data=plot, x="Item Degree", bins=bins, log_scale=(False, True))
        plt.savefig(
            os.path.join(dirname, name + "_i_degree.png"), dpi=300, bbox_inches="tight"
        )
        plt.clf()
        ...
        if plot["User Degree"].max() < 100:
            bins = "auto"
        sns.histplot(data=plot, x="User Degree", bins=bins, log_scale=(False, True))
        plt.savefig(
            os.path.join(dirname, name + "_u_degree.png"), dpi=300, bbox_inches="tight"
        )
        plt.clf()
...
def group_plot(df, y, dirname, filename, title=None):
    ...
    plt.savefig(os.path.join(dirname, filename + ".png"), dpi=300, bbox_inches="tight")
    plt.clf()
