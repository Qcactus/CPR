...
class CPR(BPR):
    ...
    def create_mf_loss(self, args):
        ...
        u_splits = tf.split(self.batch_u_embeds,
                            self.sampler.batch_total_sample_sizes, 0)
        i_splits = tf.split(batch_pos_i_embeds,
                            self.sampler.batch_total_sample_sizes, 0)
        for idx in range(len(self.sampler.batch_total_sample_sizes)):
            ...
            if args.inference_type == "inner_product":
                pos_scores.append(
                    tf.reduce_mean(
                        [inner_product(u, i) for u, i in zip(u_list, i_list)],
                        axis=0))
                neg_scores.append(
                    tf.reduce_mean([
                        inner_product(u, i)
