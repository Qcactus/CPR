...
class UBPR(BPR):
    ...
    def create_mf_loss(self, args):
        ...
        self.p_scores = np.power(self.dataset.i_degrees /
                                 self.dataset.i_degrees.max(),
                                 args.ps_pow,
                                 dtype=np.float32)
        ...
        self.mf_loss = ubpr_loss(self.batch_u_embeds, batch_i_embeds,
                                 batch_j_embeds, batch_i_p_scores,
                                 batch_j_p_scores, self.batch_j_labels, args)
