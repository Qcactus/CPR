...
def ubpr_loss(u_embeds, i_embeds, j_embeds, i_p_scores, j_p_scores, j_labels,
              args):
    pairwise_obj = tf.reduce_sum(u_embeds * i_embeds - u_embeds * j_embeds,
                                 axis=1)
    loss = -(1 / i_p_scores) * (
        1 - (j_labels / j_p_scores)) * tf.log_sigmoid(pairwise_obj)
    loss = tf.clip_by_value(loss,
                            clip_value_min=args.clip,
                            clip_value_max=10e5)
...
def cpr_loss(pos_scores, neg_scores, args):
    ...
    if args.sample_rate == 1:
        ...
        cpr_obj_neg, _ = tf.math.top_k(-cpr_obj,
                                       k=args.batch_size,
                                       sorted=False)
...
def dcor(x, y):
    ...
    A = a - tf.reduce_mean(a, axis=0)[None, :] - tf.reduce_mean(
        a, axis=1)[:, None] + tf.reduce_mean(a)
    B = b - tf.reduce_mean(b, axis=0)[None, :] - tf.reduce_mean(
        b, axis=1)[:, None] + tf.reduce_mean(b)
