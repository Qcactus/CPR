...
def create_evaluators(dataset, eval_types, metrics, ks, n_thread):
    ...
    for eval_type in eval_types:
        evaluators[eval_type] = Evaluator(dataset, eval_type, metrics, ks,
                                          (eval_type == "valid"), n_thread)
    return evaluators
...
class Evaluator(CyEvaluator):
    def __init__(
        ...
        if group:
            ...
            self.u_groups = dataset.u_groups[
                self.eval_users] if dataset.u_groups is not None else None
