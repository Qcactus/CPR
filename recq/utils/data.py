...
class BPRSampler(object):
    ...
    def set_samplers(self, args):
        ...
        self.neg_i_sampler = CyPairNegSampler(self.train, self.neg_items,
                                              self.n_step,
    ...
    def sample(self):
        ...
        rand_items = np.random.choice(self.n_item,
                                      size=self.choice_size).astype(np.int32)
...
class UBPRSampler(BPRSampler):
    def set_samplers(self, args):
        ...
        self.labeler = CyLabeler(self.train, self.j_labels, self.n_step,
                                 self.batch_sample_size, args.n_thread)

    def sample(self):
        idx = np.random.choice(self.n_interact, size=self.sample_size)
        users = self.u_interacts[idx]
        i_items = self.i_interacts[idx]
        j_items = np.random.choice(self.n_item,
                                   size=self.sample_size).astype(np.int32)
...
class CPRSampler(BPRSampler):
    def set_samplers(self, args):
        ratios = np.power(args.sample_ratio,
                          np.arange(args.max_k_interact - 2, -1, -1))
        batch_sizes = np.round(args.batch_size / np.sum(ratios) *
                               ratios).astype(np.int32)
        batch_sizes[-1] = args.batch_size - np.sum(batch_sizes[:-1])
        self.batch_sample_sizes = np.ceil(
            np.array(batch_sizes) * args.sample_rate).astype(np.int32)
        self.batch_total_sample_sizes = self.batch_sample_sizes * np.arange(
            2,
            len(self.batch_sample_sizes) + 2)
        ...
        self.cpr_sampler = CyCPRSampler(self.train, self.u_interacts,
                                        self.i_interacts, self.users,
                                        self.items, self.n_step,
                                        self.batch_sample_sizes, args.n_thread)

    def sample(self):
        interact_idx = np.random.choice(self.n_interact,
                                        size=self.choice_size).astype(np.int32)

        if self.cpr_sampler.sample(interact_idx,
                                   self.batch_choice_sizes) == -1:
            raise RuntimeError("choice_size is not large enough")

        return zip(batch_iterator(self.users, self.batch_sample_size),
